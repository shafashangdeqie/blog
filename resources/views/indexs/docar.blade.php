...
<html lang="zh-cn">
  ...
  <body>
    <div class="maincont">
     ...
     @foreach($car as $key=>$val)
      <table>
       <tr>
        ...
        <td width="50%">
         <h3>{{$val->username}}</h3>
         <input type="hidden" name="p_id" id="p_id" p_id="{{$val->p_id}}">
         <time>下单时间：2015-08-11  13:51</time>
        </td>
        <td align="right"><input type="text" class="spinnerExample" /></td>
       </tr>
       <tr>
        <th colspan="4"><strong class="orange">¥{{$val->pirce}}</strong></th>
       </tr>
       <tr>
        <td width="100%" colspan="4">
        <a href="delcar{{$val->cid}}">
     ...
     <table>
      <tr>
       ...
       <td width="40%">
       <a href="successdo{{$val->p_id}}" class="jiesuan">去结算</a>
       </td>
      </tr>
     </table>
    </div><!--gwcpiao/-->
   ...
   <script>
      ...
      var p_id = $("input[type='hidden']").attr('p_id');
        }
