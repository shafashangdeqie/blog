...
<html lang="zh-cn">
  ...
  <body>
    <div class="maincont">
     ...
     <div class="dingdanlist">
      ...
      @foreach($data as $key=>$val)
       ...
       <input type="hidden" id="s_id" s_id="{{$val->s_id}}">
       </td>

        <td width="50%">
         <h3>订单号：{{$val->s_sand}}</h3>
         <time>创建日期：{{ date( "Y-m-d H : i : s", $val->c_time)}}<br />
          失效日期：{{ date( "Y-m-d H : i : s", $val->s_time)}}</time>
         <strong class="orange">¥{{$val->s_price}}</strong>
        </td>
    ...
    <div class="gwcpiao">
     <table>
      <tr>
       <td width="50%"><a href="prolist" class="jiesuan" style="background:#5ea626;">继续购物</a>
       </td>
       <td width="10%">
       <a href="/pay{{$val->s_sand}}" id="zhifubao"  class="jiesuan">支付宝支付</a>
       </td>
       <td width="10%">
       <a href="success.html" class="jiesuan">微信支付</a>
       </td>
       <td width="10%">
       <a href="success.html" class="jiesuan">银行卡支付</a>
       </td>
      </tr>
     </table>
    </div><!--gwcpiao/-->
